...
- Still open: opaque `repository_context` records, clean-environment Postgres/Redis integration run, staging cookie-auth integration, backup/restore rehearsal, lint ratchet, EU AI Act legal review. Frontend and backend lockfile audits pass; Alembic offline coverage is green and the configured Neon OnRamp schema is verified at `031_embedding_chunk_tenant_scope`. No deployment or traffic cutover has been performed.
...
| Playbooks | ✅ Complete |
| Razorpay Billing (INR) | 🟡 Integration-ready — signature verification, idempotency, and webhook handling are implemented; live merchant credentials, reconciliation, and production webhook verification remain deployment tasks |
...
|-------|----------|--------|
| Backend (auth middleware, rate limit, RBAC guard, billing webhook, storage `in` filter, migration ordering, prod env validation, ramp/review-ops/benchmark, repo index, embeddings, DORA, RBAC) | `backend/tests/` — 63 files, 700+ tests | ✅ 1,252 passed, 202 skipped in memory mode; clean Postgres run remains a separate CI gate |
...
| Rotate DB password + prod secret manager | High | Local `.env` contains a database credential; rotate it and move production secrets to a secret manager |
| `DateTime(timezone=True)` migration | ✅ Applied | Timezone-aware columns are in the Alembic chain (migration 005) |
| Alembic model/schema parity | Medium | `alembic current` is at `031`; `alembic check` still reports pre-existing index/constraint/nullability drift requiring a deliberate follow-up migration |
...
**Next:** run the 5-team validation interviews (`docs/validation-interview-script.md`), complete a backup/restore rehearsal and deployed smoke check, run Razorpay test-mode E2E, and complete production secret rotation
