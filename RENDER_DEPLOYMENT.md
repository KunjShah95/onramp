...
Onramp runs as **5 Render services** (defined in `render.yaml`):
...
5. **Web Service** (`onramp-n8n`) — n8n automation bus (`Dockerfile.n8n`), health check `/healthz`, disk 10GB at `/home/node/.n8n`

All five share the `onramp-shared` env group so secrets stay in sync.
Frontend stays on Vercel (`VITE_API_URL=https://<onramp-api>.onrender.com/api/v1`,
`CORS_ALLOWED_ORIGINS` must include the Vercel URL); n8n ↔ backend is wired via
`N8N_*` vars below + Settings → Integrations → n8n (no restart needed).
...
### n8n automation bus (two-way: Onramp <-> n8n)
```
# Backend (onramp-shared group — global fallback; per-user override needs no restart):
N8N_WEBHOOK_URL=https://onramp-n8n.onrender.com/webhook/onramp
...
N8N_ENCRYPTION_KEY=<auto-generated>  # generateValue:true in render.yaml
N8N_INBOUND_SECRET=<same as backend>
```
Wire-up: import `n8n/workflows/*.json` in the n8n UI → set Slack OAuth2 redirect
`https://<your-n8n>.onrender.com/rest/oauth2-credential/callback` → Activate →
copy production `/webhook/onramp` URL → Onramp Settings → Integrations → n8n →
Test → Connect → Fire event. Inbound: n8n HTTP node POSTs
`{BACKEND_URL}/api/v1/webhooks/n8n` with `X-N8N-Signature`.
Verify: `GET /integrations/n8n/status` → `env_configured:true, inbound_configured:true`.
