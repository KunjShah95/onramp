    ...
    <section className="rounded-tile bg-bg-primary border border-border p-4 shadow-card">
      <div className="flex items-center justify-between gap-3 mb-3">
        <div className="flex items-center gap-2">
          <span className="material-symbols-outlined text-lg text-accent-from">science</span>
          <h2 className="text-body-sm font-semibold text-text-primary">Cost model · Phase 0</h2>
          <span className={cn(
            'px-1.5 py-0.5 rounded-md text-caption font-medium',
            source === 'team' ? 'bg-info/10 text-info' : 'bg-bg-tertiary text-text-muted'
          )}>
            {source === 'team' ? 'team-calibrated' : 'platform default'}
          </span>
        </div>
        {canEdit && !editing && (
          <button onClick={openEdit} className="text-caption text-text-muted hover:text-text-primary transition-colors">
            Calibrate
          </button>
        )}
      </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-3">
        <div className="rounded-tile bg-bg-tertiary/40 border border-border p-2.5">
          <div className="text-caption text-text-muted mb-0.5">Senior rate</div>
          <div className="readout text-body font-semibold tabular-nums text-text-primary">
            {fmtUsd(settings.senior_hourly_rate_usd)}<span className="text-caption text-text-muted">/hr</span>
          </div>
        </div>
        <div className="rounded-tile bg-bg-tertiary/40 border border-border p-2.5">
          <div className="text-caption text-text-muted mb-0.5">Review cycle</div>
          <div className="readout text-body font-semibold tabular-nums text-text-primary">
            {fmtH(settings.review_hours_per_cycle)}
          </div>
        </div>
        <div className="rounded-tile bg-bg-tertiary/40 border border-border p-2.5">
          <div className="text-caption text-text-muted mb-0.5">Stalled re-engagement</div>
          <div className="readout text-body font-semibold tabular-nums text-text-primary">
            {fmtH(settings.stalled_weekly_hours)}<span className="text-caption text-text-muted">/wk</span>
          </div>
        </div>
        <div className="rounded-tile bg-bg-tertiary/40 border border-border p-2.5">
          <div className="text-caption text-text-muted mb-0.5">Onramp price</div>
          <div className="readout text-body font-semibold tabular-nums text-text-primary">
            {fmtUsd(settings.onramp_price_usd_per_month)}<span className="text-caption text-text-muted">/mo</span>
          </div>
          <div className="text-caption text-text-muted/70">
      ...
      <div className="text-caption text-text-muted mb-3">
        Estimate{' '}
        <span className="text-text-primary font-medium readout tabular-nums">{fmtUsd(band.cost_current)}</span>
        {' '}· honest range{' '}
        <span className="text-text-primary tabular-nums">{fmtUsd(band.cost_low)}–{fmtUsd(band.cost_high)}</span>
        <span className="text-text-muted/70"> (rate $75–100 · cycle 0.25–1h)</span>
      </div>

      {/* Measured vs assumption */}
      <div className="space-y-1 text-caption text-text-muted font-code">
        <div className="flex items-center gap-2">
          <span>measured avg review cycle</span>
          <span className="readout text-text-primary tabular-nums">
        ...
        <div className="mt-3 pt-3 border-t border-border">
          <div className="flex items-center justify-between gap-3 mb-1.5">
            <p className="text-caption text-text-muted">
              vs Onramp <span className="text-text-primary font-medium readout">{fmtUsd(bench.current.onramp_cost_usd)}</span>
              <span className="text-text-muted/70"> at {fmtOnrampPrice(bench.current.onramp_price_usd_per_month, bench.current.onramp_price_inr)} (workspace)</span>
              <span className="ml-2 px-1.5 py-0.5 rounded-md bg-bg-tertiary text-text-muted text-[11px]">
                {benchStack === 'react' ? 'React-scoped' : `team-wide · ${bench.current.team_stack}`}
              </span>
              <span className={cn(
                'ml-1 px-1.5 py-0.5 rounded-md text-[11px]',
                bench.current.price_source === 'subscription' ? 'bg-go/10 text-go' : 'bg-bg-tertiary text-text-muted'
              )}>
          ...
          <p className="text-body-xs text-text-primary">
                ...
                <div key={s.generated_at} className="flex items-center justify-between text-caption text-text-muted font-code">
                  ...
                  <span>
                    ...
                    {s.onramp_price_inr != null && s.onramp_price_inr > 0 && (
                      <span className="text-text-muted/70"> · {fmtOnrampPrice(s.onramp_price_usd_per_month ?? s.onramp_cost_usd, s.onramp_price_inr)}</span>
                    )}
                  </span>
                </div>
              ))}
        ...
        <div className="mt-3 pt-3 border-t border-border space-y-2.5">
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
            <label className="block">
              <span className="text-caption text-text-muted block mb-1">Senior rate ($/hr, 20–1000)</span>
              <input
                type="number"
                value={rateDraft}
            ...
            <label className="block">
              <span className="text-caption text-text-muted block mb-1">Review cycle (h, 0.05–8)</span>
              <input
                type="number"
                step="0.05"
                value={cycleDraft}
            ...
            <label className="block">
              <span className="text-caption text-text-muted block mb-1">Onramp price ($/mo workspace, 0.05–1000)</span>
              <input
                type="number"
                step="0.05"
                value={priceDraft}
          ...
          {updateMutation.isError && (
            <p className="text-caption text-error">{String(updateMutation.error)}</p>
          )}
          <div className="flex gap-2">
            <button onClick={save} disabled={updateMutation.isPending} className="btn">
              {updateMutation.isPending ? 'Saving…' : 'Save'}
            </button>
            <button onClick={() => setEditing(false)} className="text-caption text-text-muted hover:text-text-primary">
              Cancel
            </button>
          </div>
        </div>
