...
export default function NotificationBell() {
  ...
  const typeIcons: Record<string, Icon> = {
    task_assigned: ClipboardText,
    task_started: Play,
    task_submitted: Star,
    task_reviewed: Eye,
    task_approved: CheckCircle,
    task_needs_changes: NotePencil,
    task_completed: Confetti,
    task_cancelled: XCircle,
    module_granted: LockOpen,
    team_invite: UserPlus,
    system_alert: WarningCircle,
    pr_merged: GitMerge,
    milestone_reached: Flag,
    quiz_graded: Question,
    dev_stuck: UserFocus,
  }
  ...
  return (
    <div ref={containerRef} className="relative">
      ...
      >
        {unreadCount > 0 ? (
          <BellRinging size={18} weight="fill" aria-hidden className="shrink-0" />
        ) : (
          <Bell size={18} aria-hidden className="shrink-0" />
        )}
        {unreadCount > 0 && (
          <span className="absolute -top-0.5 -right-0.5 w-[18px] h-[18px] flex items-center justify-center rounded-full bg-abort text-white text-[9px] font-bold leading-none min-w-[18px] min-h-[18px]">
          ...
          <div className="overflow-y-auto max-h-[400px]">
            {loading && notifications.length === 0 && (
              <div className="flex items-center justify-center py-8">
                <Spinner size={20} aria-hidden className="animate-spin text-go" />
              </div>
            )}
                  ...
                  {(() => {
                    ...
                    return (
                      <TypeIcon
                        size={18}
                        aria-hidden
                        className={cn('mt-0.5 shrink-0', typeColors[n.type] || 'text-ink-muted')}
                      />
                    )
                  })()}
