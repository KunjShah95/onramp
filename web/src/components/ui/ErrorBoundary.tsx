    ...
    if (this.state.hasError) {
      ...
      return (
        <div
          ...
          <h3 className="text-base font-semibold text-text-primary mb-1" id="error-boundary-heading">Something went wrong</h3>
          <p className="text-sm text-text-muted text-center max-w-md mb-4">
            {this.state.error?.message || 'An unexpected error occurred while rendering this section.'}
          </p>
          <button
            onClick={this.handleRetry}
            className="px-4 py-2 bg-warning hover:bg-warning-lit text-[hsl(var(--primary-foreground))] text-xs font-bold rounded-btn transition-colors"
          >
            Try Again
          </button>
        </div>
