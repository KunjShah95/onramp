...
export default function LandingPageV3() {
  ...
  return (
    ...
    >
      ...
      {/* ═══ Hero ═══ */}
      <section ref={heroRef} className="relative overflow-hidden pt-[128px] pb-24" style={{ background: '#050816' }}>
        {/* Atmospheric mountain photo, deeply dimmed with dark overlay */}
        <motion.div
        ...
        >
          ...
          />
          {/* indigo/cyan rim light behind copy */}
          <div className="absolute inset-0" style={{ background: 'radial-gradient(70% 60% at 28% 42%, rgba(79,70,229,.20) 0%, rgba(34,211,238,.08) 35%, transparent 65%)' }} />
          <div className="absolute inset-0" style={{ background: 'linear-gradient(to bottom, #050816 0%, transparent 20%, transparent 80%, #050816 100%)' }} />
        </motion.div>
        ...
        <div className="relative z-10 mx-auto grid max-w-[1280px] grid-cols-1 items-center gap-14 px-6 lg:grid-cols-[minmax(0,0.9fr)_minmax(0,1.1fr)] lg:px-10">
          ...
          <div className="max-w-xl">
            <motion.div
              {...fadeUp(0)}
              className="inline-flex items-center gap-2 rounded-full border border-white/10 bg-white/5 px-3.5 py-1.5 text-[13px] font-medium text-[#94A3B8] backdrop-blur-sm"
            >
              <Sparkle size={13} weight="fill" className="text-[#22D3EE]" />
              The AI-powered developer onboarding platform
            </motion.div>

            <motion.h1
              {...fadeUp(0.08)}
              className="mt-6 font-display text-[52px] leading-[0.98] tracking-[-0.02em] text-white sm:text-[64px]"
            >
              Onboard developers in hours, not{' '}
              <span className="italic text-[#22D3EE]">weeks.</span>
            </motion.h1>

            <motion.p
              {...fadeUp(0.16)}
              className="mt-6 max-w-md text-[17px] leading-[1.6] text-[#94A3B8]"
            >
            ...
            <motion.div {...fadeUp(0.24)} className="mt-8 flex flex-wrap items-center gap-3">
              <button
                ...
                className="group inline-flex items-center gap-2 rounded-[12px] bg-gradient-to-r from-[#4F46E5] to-[#6366F1] px-6 py-3.5 text-[15px] font-medium text-white shadow-[0_2px_16px_rgba(79,70,229,.35)] transition-all hover:shadow-[0_2px_24px_rgba(79,70,229,.50)] active:scale-[0.98]"
              >
                Start free
              ...
              <button
                ...
                className="inline-flex items-center gap-2 rounded-[12px] border border-white/10 bg-white/5 px-6 py-3.5 text-[15px] font-medium text-white transition-all hover:bg-white/10 active:scale-[0.98]"
              >
                <Play size={15} weight="fill" className="text-[#22D3EE]" />
                Watch demo
              </button>
            </motion.div>
            ...
            <motion.div
              {...fadeUp(0.32)}
              className="mt-8 flex flex-wrap items-center gap-x-6 gap-y-3 text-[13px] font-medium text-[#94A3B8]"
            >
              {[
                ...
                <span key={label} className="inline-flex items-center gap-1.5">
                  <Icon size={15} weight="duotone" className="text-[#22D3EE]" />
                  {label}
                </span>
              ))}
            </motion.div>
...
function DashboardMockup() {
  return (
    <div className="overflow-hidden rounded-[16px] border border-white/10 bg-[#0A0E1A] shadow-[0_2px_4px_rgba(0,0,0,.20),0_40px_80px_-24px_rgba(0,0,0,.50)]">
      <div className="flex">
        {/* Sidebar */}
        <aside className="hidden w-[190px] shrink-0 flex-col gap-1 border-r border-white/5 bg-white/[0.03] p-3 sm:flex">
          <div className="mb-3 flex items-center gap-2 px-2 py-1.5">
            ...
            <span className="text-[13px] font-semibold text-white">Onramp</span>
          </div>
          {[
            ...
            <div
              ...
              className={`flex items-center gap-2.5 rounded-[8px] px-2.5 py-1.5 text-[12.5px] ${
                n.active ? 'bg-indigo-500/15 font-medium text-[#818CF8]' : 'text-[#64748B]'
              }`}
            >
        ...
        <div className="min-w-0 flex-1 p-4">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              <span className="hidden items-center gap-1.5 rounded-[7px] border border-white/10 bg-white/[0.03] px-2 py-1 text-[11px] text-[#94A3B8] sm:flex">
                <GithubLogo size={12} weight="fill" /> acme/platform
              </span>
              <span className="h-6 w-6 rounded-full bg-[#6366F1]/30" />
            </div>
          </div>

          <div className="mt-3 text-[15px] font-semibold text-white">Good morning, Alex 👋</div>
          <div className="text-[11px] text-[#64748B]">Here&rsquo;s what&rsquo;s happening with your codebase.</div>

          {/* Stat cards */}
          <div className="mt-3 grid grid-cols-2 gap-2 lg:grid-cols-4">
            {[
              { k: 'Codebase health', v: '92%', t: 'Healthy', c: '#22D3EE' },
              { k: 'Architecture score', v: '89/100', t: 'Good', c: '#818CF8' },
              { k: 'Active onboarding', v: '12', t: 'Engineers', c: '#F59E0B' },
              { k: 'Questions answered', v: '1,248', t: 'This week', c: '#818CF8' },
            ].map((s) => (
              <div key={s.k} className="rounded-[10px] border border-white/5 bg-white/[0.03] p-2.5">
                <div className="text-[9.5px] text-[#64748B]">{s.k}</div>
                <div className="mt-1 text-[16px] font-semibold text-white">{s.v}</div>
          ...
          <div className="mt-2.5 grid grid-cols-1 gap-2.5 lg:grid-cols-3">
            {/* Architecture map */}
            <div className="rounded-[10px] border border-white/10 bg-[#0D1117] p-2.5">
              <div className="text-[10px] font-medium text-[#94A3B8]">Architecture map</div>
              <div className="relative mt-2 h-[92px]">
                {[
                  ...
                  <div
                    key={n.l}
                    className="absolute rounded-[6px] border border-white/10 bg-[#0A0E1A] px-1.5 py-1 shadow-[0_1px_2px_rgba(0,0,0,.20)]"
                    style={{ left: n.x, top: n.y }}
                  >
                    <div className="text-[8px] font-semibold text-white">{n.l}</div>
                    <div className="text-[7px] text-[#64748B]">{n.s}</div>
                  </div>
                ))}
                <span className="absolute left-[24%] top-[62%] h-1.5 w-1.5 rounded-full bg-[#818CF8]" />
                <span className="absolute left-[50%] top-[52%] h-1.5 w-1.5 rounded-full bg-[#818CF8]" />
              </div>
            </div>

            {/* Recent activity */}
            <div className="rounded-[10px] border border-white/5 bg-white/[0.03] p-2.5">
              <div className="text-[10px] font-medium text-[#94A3B8]">Recent activity</div>
              <div className="mt-2 space-y-2">
                {[
                  { c: '#22D3EE', t: 'New engineer onboarded', s: '2h ago' },
                  { c: '#818CF8', t: 'Codebase indexed', s: '5h ago' },
                  { c: '#F59E0B', t: 'Architecture updated', s: '1d ago' },
                  { c: '#22D3EE', t: 'Docs generated', s: '2d ago' },
                ].map((a) => (
                  <div key={a.t} className="flex items-center gap-2">
                    <span className="h-1.5 w-1.5 shrink-0 rounded-full" style={{ background: a.c }} />
                    <span className="min-w-0 flex-1 truncate text-[9px] text-[#94A3B8]">{a.t}</span>
                    <span className="text-[8px] text-[#64748B]">{a.s}</span>
                  </div>
                ))}
              </div>
            </div>
            ...
            {/* Ask Onramp */}
            <div className="rounded-[10px] border border-white/5 bg-white/[0.03] p-2.5">
              <div className="text-[10px] font-medium text-[#94A3B8]">Ask Onramp</div>
              <div className="mt-2 space-y-1.5">
                {['How does authentication work?', 'Where is billing handled?', 'Explain the data flow for orders'].map((q) => (
                  <div key={q} className="flex items-center justify-between rounded-[6px] border border-white/5 bg-white/[0.03] px-2 py-1.5">
                    <span className="truncate text-[9px] text-[#94A3B8]">{q}</span>
                    <ArrowRight size={9} className="text-[#64748B]" />
                  </div>
                ))}
                <div className="flex items-center justify-between rounded-[6px] bg-[#4F46E5]/20 px-2 py-1.5">
                  <span className="text-[9px] text-white/60">Ask anything about your codebase…</span>
                  <PlusCircle size={11} weight="fill" className="text-[#22D3EE]" />
                </div>
              </div>
            </div>
          </div>
